...
namespace ServerBackupTool.API.Services
{
    public class LogPollingService : BackgroundService
    {
        ...
        private async Task PollAndDispatch(CancellationToken stoppingToken)
        {
            ...
            {
                IWebhookRegistrationService registrationService = scope.ServiceProvider.GetRequiredService<IWebhookRegistrationService>();
                IWebhookDispatchService dispatchService = scope.ServiceProvider.GetRequiredService<IWebhookDispatchService>();
